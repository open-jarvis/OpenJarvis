...
Instead of embedding all server definitions as an inline JSON string in `config.toml`, you can point to an external JSON file. This is especially useful when managing many servers or when the configuration is generated by another tool.

### Relative path

A relative path is resolved relative to the directory containing `config.toml`:
...
[tools.mcp]
enabled = true
servers = "/etc/openjarvis/mcp-servers.json"
```

### JSON file format

The referenced JSON file uses the same schema as the inline variant -- a JSON array of server objects:

```json
[
  {
    "name": "homeassistant",
    "url": "http://192.168.1.100:9583/mcp",
    "include_tools": ["ha_search_entities", "ha_call_service"]
  },
  {
    "name": "google-workspace",
    "url": "http://192.168.1.101:8000/mcp",
    "include_tools": ["list_calendars", "get_events"]
  }
]
```

A single server object without the array brackets is also accepted and will be automatically wrapped in a list:
...
Relative paths are resolved relative to the directory containing `config.toml`. For security, relative paths must stay within (or below) the config directory. Paths that traverse upward (e.g., `../../secrets.json`) are rejected. Absolute paths are allowed without restriction.

!!! note
    OpenJarvis distinguishes between an inline JSON string (starts with `[` or `{`) and a file reference (everything else). If your `servers` value does not start with `[` or `{`, it is treated as a file path.
