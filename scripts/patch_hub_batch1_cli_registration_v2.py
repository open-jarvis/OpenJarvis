...
# Remove previous loose safe-import marker block if present.
cleaned = []
skip = 0
for line in lines:
    if skip:
        skip -= 1
        continue
    if line.strip() == "# Serena Hub / Command Center CLI":
        skip = 4
        continue
    cleaned.append(line)
lines = cleaned

# Insert import after crm import, or after membership import as fallback.
if not any(line.strip() == "from openjarvis.cli.hub_cmd import hub" for line in lines):
    inserted = False
    for i, line in enumerate(lines):
        if line.strip() == "from openjarvis.cli.crm_cmd import crm":
            lines.insert(i + 1, "from openjarvis.cli.hub_cmd import hub")
            inserted = True
            break
    if not inserted:
        for i, line in enumerate(lines):
            if line.strip() == "from openjarvis.cli.membership_cmd import membership":
                lines.insert(i + 1, "from openjarvis.cli.hub_cmd import hub")
                inserted = True
                break
    if not inserted:
        raise SystemExit("[ERROR] Could not find a safe import location near CRM or Membership.")

# Insert add_command after crm add_command, or after membership as fallback.
if not any(line.strip() == 'cli.add_command(hub, "hub")' for line in lines):
    inserted = False
    for i, line in enumerate(lines):
        if line.strip() == 'cli.add_command(crm, "crm")':
            lines.insert(i + 1, 'cli.add_command(hub, "hub")')
            inserted = True
            break
    if not inserted:
        for i, line in enumerate(lines):
            if line.strip() == 'cli.add_command(membership, "membership")':
                lines.insert(i + 1, 'cli.add_command(hub, "hub")')
                inserted = True
                break
    if not inserted:
        raise SystemExit("[ERROR] Could not find a safe add_command location near CRM or Membership.")

init_path.write_text("\n".join(lines).rstrip() + "\n", encoding="utf-8")

print("[OK] Rewrote hub_cmd.py as Click CLI")
print("[OK] Registered import: from openjarvis.cli.hub_cmd import hub")
print('[OK] Registered command: cli.add_command(hub, "hub")')