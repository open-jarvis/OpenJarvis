...
cli_init_path = ROOT / "src" / "openjarvis" / "cli" / "__init__.py"

for path in [tool_path.parent, cli_path.parent]:
...
cli_code = r'''
"""CLI commands for Serena Hub / Command Center."""
...
app = typer.Typer(help="Serena Hub / Command Center local-first operator.")
...
@app.command("chat-request")
def chat_request(message: str, operator: str = "hub") -> None:
    """Create local Hub chat request artifact only."""
    _print(hub_chat_request(message=message, operator=operator))


@app.command("activity-event")
def activity_event(
    operator: str,
    event_type: str,
    message: str,
...
@app.command("action-routing-plan")
def action_routing_plan(scope: str, include_sensitive: bool = False) -> None:
...
@app.command("blocked-unapproved-action")
def blocked_unapproved_action(
    action: str,
    operator: str = "hub",
    reason: str = "Unapproved live action blocked by Hub v1.",
) -> None:
...
tool_path.write_text(dedent(tool_code).lstrip(), encoding="utf-8")
cli_path.write_text(dedent(cli_code).lstrip(), encoding="utf-8")

if cli_init_path.exists():
    init_text = cli_init_path.read_text(encoding="utf-8")
else:
    init_text = ""

if "hub_cmd" not in init_text and "hub" not in init_text:
    init_text = init_text.rstrip() + "\n\n# Serena Hub / Command Center CLI\ntry:\n    from openjarvis.cli import hub_cmd  # noqa: F401\nexcept Exception:\n    pass\n"
    cli_init_path.write_text(init_text + "\n", encoding="utf-8")

print("[OK] Wrote src/openjarvis/tools/serena_hub.py")
print("[OK] Wrote src/openjarvis/cli/hub_cmd.py")
print("[OK] Added safe hub_cmd import marker to src/openjarvis/cli/__init__.py if missing")