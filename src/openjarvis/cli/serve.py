...
def serve(
    ...
    # Storage-backed tools, API routes, prompt context, and the scheduler all
    # share one process-owned memory backend. Build it before resolving tools so
    # retrieval/memory tools never receive a null backend.
    memory_backend = None
    try:
        ...
        try:
            ...
            if AgentRegistry.contains(agent_key):
                ...
                if getattr(agent_cls, "accepts_tools", False):
                    ...
                    resolved_tools = agent_bundle.tools
