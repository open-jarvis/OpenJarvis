...
async def transcribe_speech(request: Request):
    ...
    if ext.lower() not in ("wav",):
        ...
        if wav_bytes is None:
            raise HTTPException(
                status_code=400,
                detail=(
                    "Could not decode audio. Install ffmpeg and ensure it is on PATH."
                ),
            )
    ...
    try:
        result = await asyncio.to_thread(
            ...
            audio_bytes,
            format=ext,
            # Let the backend auto-detect unless the request explicitly chose
            # a language.  ``lang`` remains the response/config fallback.
