...
class TestAuthorizationHeader:
    ...
    def test_ovms_env_var_fallback(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with respx.mock:
            route = respx.get("http://testhost:8001/v3/models").mock(
                return_value=httpx.Response(200, json={"data": []})
            )
            engine.health()
        assert route.calls.last.request.headers["Authorization"] == "Bearer sk-ovms"

    def test_ovms_uses_v3_chat_endpoint(self) -> None:
        engine = OVMSEngine(host="http://testhost:8001")
        with respx.mock:
            route = respx.post("http://testhost:8001/v3/chat/completions").mock(
                return_value=httpx.Response(200, json=_CHAT_RESPONSE)
            )
            engine.generate([Message(role=Role.USER, content="hi")], model="m")
        assert route.calls.last.request.url.path == "/v3/chat/completions"
