...
def _openai_tool_call_delta(
    *, index: int = 0, tc_id: str = "", name: str = "", arguments: str = "",
) -> MagicMock:
    tc = MagicMock()
    tc.index = index
    tc.id = tc_id
...
async def test_stream_full_openai_content():
    ...
    async for sc in engine._stream_full_openai(
        msgs, model="gpt-4o", temperature=0.7, max_tokens=100,
    ):
        result.append(sc)
...
async def test_stream_full_anthropic_content():
    ...
    async for sc in engine._stream_full_anthropic(
        msgs, model="claude-sonnet-4-20250514", temperature=0.7, max_tokens=100,
    ):
        result.append(sc)
