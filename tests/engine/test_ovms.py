...
Validates that the ``/v3`` prefix is used correctly for models listing
...
class TestOVMSEngineBasics:
    ...
    def test_api_prefix(self) -> None:
        assert OVMSEngine._api_prefix == "/v3"
...
class TestOVMSGenerate:
    def test_generate_uses_v3_prefix(self, engine: OVMSEngine) -> None:
        with respx.mock:
            respx.post("http://testhost:8001/v3/chat/completions").mock(
...
class TestOVMSHealth:
    def test_health_true(self, engine: OVMSEngine) -> None:
        with respx.mock:
            respx.get("http://testhost:8001/v3/models").mock(
...
class TestOVMSListModels:
    def test_list_models_uses_v3_prefix(self, engine: OVMSEngine) -> None:
        with respx.mock:
            respx.get("http://testhost:8001/v3/models").mock(
                return_value=httpx.Response(
                    200,
